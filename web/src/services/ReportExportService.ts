...
export function buildStandaloneHtmlReport(scan: ScanRecord): string {
  ...
  const matrixRows = scan.matrix.map((row) => `<tr>
    ...
    <td>${row.skippedAfterPriorFailure ? "skipped" : row.matchesExpectation ? "match" : "mismatch"}</td></tr>`).join("");
  const findings = scan.findings.map((finding) => findingHtml(finding)).join("");
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>API AC Scanner report ${escapeHtml(scan.id)}</title>
<style>
...
<h1>API Access-Control Assessment Report</h1>
<section class="meta"><strong>${escapeHtml(scan.kind)} scan</strong><br>${escapeHtml(safeTarget(scan.target))}<br>
Report ID: ${escapeHtml(scan.id)}<br>Created: ${escapeHtml(scan.createdAt)}<br>Status: ${escapeHtml(scan.status)} / ${escapeHtml(scan.stage)}</section>
<section class="summary">${Object.entries(summary).map(([state, count]) => `<span>${escapeHtml(state)}: ${count}</span>`).join("")}</section>
${scan.warnings.map((warning) => `<div class="warning">${escapeHtml(warning)}</div>`).join("")}
${scan.matrix.length ? `<h2>Authorization matrix</h2><table><thead><tr><th>Endpoint</th><th>Identity</th><th>Expected</th><th>Actual</th><th>Result</th></tr></thead><tbody>${matrixRows}</tbody></table>` : ""}
<h2>Findings</h2>${findings || "<p>No findings were produced. Review coverage and warnings before drawing conclusions.</p>"}
<p><small>Generated by API AC Scanner V3.1. Secrets are excluded or redacted. Findings are evidence for authorized review, not proof of complete security.</small></p>
</body></html>`;
}
...
export function buildPdfReport(scan: ScanRecord): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    document.moveDown().fontSize(8).fillColor("#64748b")
      .text("Generated by API AC Scanner V3.1. Secrets are excluded or redacted. Findings require authorized review.");
    document.end();
  });
}
...
function findingHtml(finding: Finding): string {
  ...
  return `<article class="finding"><h3>[${escapeHtml(finding.state)}] ${escapeHtml(finding.title)}</h3>
  <p>${escapeHtml(finding.owaspId)} / ${escapeHtml(finding.severity)} / ${escapeHtml(finding.confidence)} confidence</p>
  ${finding.location ? `<code>${escapeHtml(finding.location)}</code>` : ""}<p>${escapeHtml(finding.description)}</p>
  <dl>${evidence}</dl><strong>Recommendation</strong><p>${escapeHtml(finding.recommendation)}</p></article>`;
}
